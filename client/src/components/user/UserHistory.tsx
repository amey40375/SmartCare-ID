...
import { getOrders, getCurrentUser, getUsers } from '@/utils/localStorage';
import type { Order, User } from '@/utils/localStorage';
...
const UserHistory = () => {
  ...
  const getMitraName = (mitraId: string) => {
    const mitra = users.find(u => u.id === mitraId);
    return mitra?.name || 'Unknown';
  };
      ...
      <CardContent className="p-4">
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            <p className="text-sm text-muted-foreground">🧑‍🔧 Mitra: {getMitraName(order.mitraId || '')}</p>
              ...
              {orders.length === 0 ? (
                <Card>
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-muted-foreground">Mulai pesan layanan pertama Anda</p>
                    <Button 
                      className="mt-4"
